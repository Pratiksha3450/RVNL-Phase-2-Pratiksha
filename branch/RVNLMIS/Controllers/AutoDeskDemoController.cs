...
{
    public class AutoDeskDemoController : Controller
    {
       ...
       // [HttpPost]
       // public async Task<JsonResult> AddOrGetBucketAsync()
        public async Task<string> AddOrGetBucketAsync( string filePath)
        {
        ...
        refresh:
            ...
            dynamic buckets = await appBckets.GetBucketsAsync(null, 100).ConfigureAwait(false);
            if (buckets == null)
            {
                //create new bucket                
                appBckets.Configuration.AccessToken = oauth.access_token;
                PostBucketsPayload bucketPayload = new PostBucketsPayload(string.Format("{0}-{1}", ClientId, System.Guid.NewGuid().ToString()), null,
                  PostBucketsPayload.PolicyKeyEnum.Transient);
                var res = await appBckets.CreateBucketAsync(bucketPayload, null);
                goto refresh;
            }
            else
            {
                ...
                //get latest node
                var Buckid = nodes[0];
                string bucketKey = Buckid.id;

                //upload file to autodesk
                var fileSavePath = filePath;// Path.Combine(HttpRuntime.AppDomainAppPath, "Uploads/StripChart/3/mm.dxf");
                // get the bucket...                
                ...
                using (StreamReader streamReader = new StreamReader(fileSavePath))
                {
                    uploadedObj = await objects.UploadObjectAsync(bucketKey,
                           Path.GetFileName(fileSavePath), (int)streamReader.BaseStream.Length, streamReader.BaseStream,
                           "application/octet-stream");
                }
                var objId = Base64Encode(uploadedObj.objectId);
                // transfer file
                // prepare the payload
                ...
                JobPayload job;
                job = new JobPayload(new JobPayloadInput(objId), new JobPayloadOutput(outputs));
                ...
                while (progress < 100)
                {
                    ...
                    try
                    {
                        dynamic manifest = await derivative.GetManifestAsync(objId);
                        string status = (manifest.status);
                        if (status.Contains("cuccess"))
                        {
                            progress = 100;
                        }
                        else
        ...
        public JsonResult SaveAndLoadStraightLineFile(AutoCadViewerModel oModel)
        {
            try
            {
                if (oModel.PackageId != 0)
                {
                    ...
                    var res = Task.Run<string>(async () => await AddOrGetBucketAsync(_Plot));
                    oModel.FileName = res.Result;
            ...
            try
            {
                ...
                if (oModel.PackageId != 0)
                {
                    ...
                    else
                    {
                        ...
                        var res = Task.Run<string>(async () => await AddOrGetBucketAsync(newImage));
                        System.Threading.Thread.Sleep(5000);
                        oModel.FileName = res.Result;
                        return Json(oModel);
                    }
