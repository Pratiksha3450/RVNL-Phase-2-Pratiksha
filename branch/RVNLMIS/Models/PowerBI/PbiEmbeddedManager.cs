...
namespace RVNLMIS.Models
{
    public class PbiEmbeddedManager
    {
        ...
        public static async Task<ReportEmbeddingData> GetReportEmbeddingDataWithSecurity(string ID, string Username, string Role)
        {
            ...
            var embedUrl = report.EmbedUrl;
            var reportName = report.Name;
            string accessLevel = HttpContext.Current.User.IsInRole("SAD") ? "edit" : "view";

            var generateTokenRequestParameters = new GenerateTokenRequest(accessLevel: accessLevel, identities: new List<EffectiveIdentity> { new EffectiveIdentity(username: Username, roles: new List<string> { Role }, datasets: new List<string> { report.DatasetId }) });
