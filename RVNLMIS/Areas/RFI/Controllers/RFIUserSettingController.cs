...
{
    public class RFIUserSettingController : Controller
    {
        // GET: Setting
        public ActionResult Index()
        {
            ...
            {
                objModel.objUser = dbContext.tblRFIUsers.Where(u => u.RFIUserId == userId)
                    .AsEnumerable()
                ...
                {
                    objModel.UserImagePath = string.Format("/Uploads/RFIProfilePictures/{0}/{1}", objModel.objUser.UserId, objModel.objUser.ProfilePic);
                }
        ...
        public ActionResult EditProfileSubmit(UserSettingModel objModel)
        {
            ...
            try
            {
                ...
                using (dbRVNLMISEntities dbContext = new dbRVNLMISEntities())
                {
                    var getObj = dbContext.tblRFIUsers.Where(u => u.RFIUserId == objModel.objUser.UserId).FirstOrDefault();
                    getObj.FullName = objModel.objUser.Name;
                    getObj.Email = objModel.objUser.EmailId;
                    getObj.Mobile = objModel.objUser.MobileNo;
                    // getObj.ProfilePic = fileName;

                    dbContext.SaveChanges();
                    return Json(new { message = "1", name = getObj.FullName, mobile = getObj.Mobile, email = getObj.Email }, JsonRequestBehavior.AllowGet);
                }
            }
            catch (Exception ex)
            {
        ...
        public JsonResult Upload()
        {
            ...
            for (int i = 0; i < Request.Files.Count; i++)
            {
                ...
                string localPath = string.Format("~/Uploads/RFIProfilePictures/{0}", userId);
                Functions.CreateIfMissing(Server.MapPath(localPath));

                string getFileName = Path.GetFileName(file.FileName);
                fileName = string.Concat(userId + "-" + getFileName.Replace(' ', '_'));
                filePath = string.Format("/Uploads/RFIProfilePictures/{0}/{1}", userId, fileName);

                file.SaveAs(Server.MapPath(filePath));

                using (dbRVNLMISEntities dbContext = new dbRVNLMISEntities())
                {
                    var getObj = dbContext.tblRFIUsers.Where(u => u.RFIUserId == userId).FirstOrDefault();
                    getObj.ProfilePic = fileName;
