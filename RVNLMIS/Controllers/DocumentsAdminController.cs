...
namespace RVNLMIS.Controllers
{
    ...
    {       
        ...
        public JsonResult GetDocumentDetails(FormCollection fc)
        {
            ...
            string UserID =  Convert.ToString(fc["UserId"]);
            UserModel objUserM = new UserModel();
            objUserM = (UserModel)Session["UserData"];

            if (string.IsNullOrEmpty(UserID))
            {
                UserIDStr = Convert.ToString(objUserM.UserId);
            }
            else
            {
                UserIDStr = UserID;
        ...
        public JsonResult GetUserList(string text)
        {
            ...
            {
                var UserList = dbContext.tblUserMasters.Where(d => d.IsDeleted == false)
                    .Select(s => new { s.UserId, s.UserName }).ToList();
                if (!string.IsNullOrEmpty(text))
                {
                    UserList = UserList.Where(p =>
                   CultureInfo.CurrentCulture.CompareInfo.IndexOf
                   (p.UserName, text, CompareOptions.IgnoreCase) >= 0).ToList();
                }
        ...
        public List<Models.FileInf> GetFile(string UserID, DateTime fromDate, DateTime toDate )
        {
            string UserIDStr;
            UserModel objUserM = new UserModel();
            objUserM = (UserModel)Session["UserData"];
            ...
            paramlist.Add(new SqlParameter("@UserID", UserIDStr));
        ...
        [HttpGet]
        public ActionResult DownloadFiles(string fromDate, string toDate, string UserId, string mode)
        {
            //string fromDate = fc["fromDate"];
            //string toDate = fc["toDate"];
            ...
            string TodaysDt = DateTime.Now.ToString();
                ...
                var filesCol = GetFile(UserId, Convert.ToDateTime(fromDate), Convert.ToDateTime(toDate)).ToList();
                if (filesCol.Count >= 1)
                {
                    ...
                    try
                    {
                        ...
                        {
                            int k = Functions.SaveUserLog(pkgId, "Documents", "Download", Convert.ToInt32(UserId), IpAddress, "NA" );
                    ...
                    {
                        if (mode == "action")
                            return Json(new { fromDate = fromDate, toDate = toDate, UserId= UserId }, JsonRequestBehavior.AllowGet);
                        else
                        { 
